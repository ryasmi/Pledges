...
/*(function () {
    ...
    describe('promise', function () {
        ...
        var testThen = function (restrict) {
            return function () {
                var myPromise, myRestrictedPromise;

                beforeEach(function () {
                    myPromise = promise();
                    myRestrictedPromise = restrict === true ? myPromise.restrict() : myPromise;
                });

                it('when fulfilled after being called should call \'onFulfilled\' callback with value', function (done) {
                    myRestrictedPromise.then(correctCallback(done), incorrectCallback);
                    myPromise.fulfil(myValue);
                });

                it('when fulfilled before being called should call \'onFulfilled\' callback with value', function (done) {
                    myPromise.fulfil(myValue);
                    myRestrictedPromise.then(correctCallback(done), incorrectCallback);
                });

                it('when rejected after being called should call \'onRejected\' callback with reason', function (done) {
                    myRestrictedPromise.then(incorrectCallback, correctCallback(done));
                    myPromise.reject(myValue);
                });

                it('when rejected before being called should call \'onRejected\' callback with reason', function (done) {
                    myPromise.reject(myValue);
                    myRestrictedPromise.then(incorrectCallback, correctCallback(done));
                });
            };
        };
        var testState = function (restrict) {
            return function () {
                var myPromise, myRestrictedPromise;
                ...
                it('when unfulfilled should return \'unfulfilled\'', function () {
                    expect(myRestrictedPromise.state()).to.equal('unfulfilled');
                });

                it('when fulfilled should return \'fulfilled\'', function () {
                    myPromise.fulfil();
                    expect(myRestrictedPromise.state()).to.equal('fulfilled');
                    myPromise.reject();
                    expect(myRestrictedPromise.state()).to.equal('fulfilled');
                });

                it('when rejected should return \'rejected\'', function () {
                    myPromise.reject();
                    expect(myRestrictedPromise.state()).to.equal('rejected');
                    myPromise.fulfil();
                    expect(myRestrictedPromise.state()).to.equal('rejected');
                });
            };
        ...
        it('should expose a \'then\' function', function () {
            expect(myPromise.then).to.be.a('function');
        });
        it('should expose a \'fulfil\' function', function () {
            expect(myPromise.fulfil).to.be.a('function');
        });
        it('should expose a \'reject\' function', function () {
            expect(myPromise.reject).to.be.a('function');
        });
        it('should expose a \'state\' function', function () {
            expect(myPromise.state).to.be.a('function');
        });
        it('should expose a \'restrict\' function', function () {
            expect(myPromise.restrict).to.be.a('function');
        });

        describe('restricted promise', function () {
            var myRestrictedPromise;

            before(function () {
                myRestrictedPromise = promise().restrict();
            });
            ...
            describe('then', testThen(true));
            describe('state', testState(true));
        });

        describe('then', testThen(false));
        describe('state', testState(false));
    });
}());*/